...
export default function Headliners() {
  ...
  const [pendingPortalooRefresh, setPendingPortalooRefresh] = useState(0); // 0=none, 1=first refresh done, 2=second refresh done
  ...
  const aiStep = () => {
    ...
    if (phase === "game") {
      ...
      if (decision.action === "drawDeck") {
        // AI draws 2, keeps best, swaps other into pool
        const drawn = drawFromDeck(2);
        if (drawn.length >= 2) {
          const best = drawn[0].vp + drawn[0].tickets >= drawn[1].vp + drawn[1].tickets ? 0 : 1;
          const kept = drawn[best]; const other = drawn[1 - best];
          setPlayerData(p => ({ ...p, [currentPlayerId]: { ...p[currentPlayerId], hand: [...p[currentPlayerId].hand, kept] } }));
          addLog("🤖 AI", `Drew ${kept.name} from deck, kept it`);
          // Swap other into pool (replace worst pool artist, or add if pool has space)
          if (artistPool.length > 0) {
            const worstIdx = artistPool.reduce((wi, a, i, arr) => (a.vp + a.tickets < arr[wi].vp + arr[wi].tickets ? i : wi), 0);
            const replaced = artistPool[worstIdx];
            const newPool = [...artistPool]; newPool[worstIdx] = other;
            setArtistPool(newPool);
            setDiscardPile(prev => [...prev, replaced]);
            addLog("🤖 AI", `Swapped ${other.name} into pool, discarded ${replaced.name}`);
          } else {
            setArtistPool(prev => [...prev, other]);
          }
        } else if (drawn.length === 1) {
          setPlayerData(p => ({ ...p, [currentPlayerId]: { ...p[currentPlayerId], hand: [...p[currentPlayerId].hand, drawn[0]] } }));
          addLog("🤖 AI", `Drew ${drawn[0].name} from deck`);
        }
        setTurnsLeft(p => ({ ...p, [currentPlayerId]: p[currentPlayerId] - 1 }));
  ...
  const handleReserveFromPool = (idx) => {
    ...
    setArtistPool(newPool); // Pool does NOT refill — stays at 4 (or fewer)
  ...
  const beginEventPhase = () => {
    ...
    // Apply year-end artist effects BEFORE events (Foo Fighters, Fleetwood Mac, Kendrick, Eminem, etc.)
    players.forEach(p => {
      const pd = playerData[p.id];
      if (!pd) return;
      (pd.stageArtists || []).forEach(sa => sa.forEach(a => {
        try {
          ...
          const pName = p.festivalName;
          
          // Foo Fighters: Roll all 5 dice, +1VP per unique amenity
          if (eff.includes("roll all") && eff.includes("unique amenity")) {
            ...
            setPlayerData(prev => ({ ...prev, [p.id]: { ...prev[p.id], vp: (prev[p.id].vp || 0) + unique } }));
            addLog("🎸 Year End", `${pName}: ${a.name} rolled ${results.map(d => d === "fame" ? "🔥" : AMENITY_ICONS[d] || d).join(" ")} → ${unique} unique = +${unique} VP!`);
            showFloatingBonus(`+${unique} ⭐ ${a.name}`, "#c4b5fd");
          }
          // Fleetwood Mac: Roll all 5 dice, +1VP per most common result
          if (eff.includes("roll all") && eff.includes("most common")) {
            const results = shuffle([...DICE_OPTIONS, ...DICE_OPTIONS]).slice(0, 5);
            const c = {}; results.forEach(d => { c[d] = (c[d] || 0) + 1; }); const best = Math.max(...Object.values(c));
            setPlayerData(prev => ({ ...prev, [p.id]: { ...prev[p.id], vp: (prev[p.id].vp || 0) + best } }));
            addLog("🎸 Year End", `${pName}: ${a.name} rolled ${results.map(d => d === "fame" ? "🔥" : AMENITY_ICONS[d] || d).join(" ")} → best streak ${best} = +${best} VP!`);
            showFloatingBonus(`+${best} ⭐ ${a.name}`, "#c4b5fd");
          }
          // Coldplay: +1 VP / Fame gained this year
          if (eff.includes("vp / fame gained")) {
            const fameGained = pd.baseFame || 0;
            if (fameGained > 0) {
              setPlayerData(prev => ({ ...prev, [p.id]: { ...prev[p.id], vp: (prev[p.id].vp || 0) + fameGained } }));
              addLog("🎸 Year End", `${pName}: ${a.name} → +${fameGained} VP (fame gained)`);
              showFloatingBonus(`+${fameGained} ⭐ ${a.name}`, "#c4b5fd");
            }
          }
          // Lady Gaga: +1 VP if highest fame, +3 VP if highest fame AND most tickets
          if (eff.includes("vp if you have the highest fame")) {
            const myFame = pd.fame || 0;
            const myTickets = pd.tickets || 0;
            ...
            if (isHighestFame) {
              ...
              if (isHighestTickets) bonus += 3;
              setPlayerData(prev => ({ ...prev, [p.id]: { ...prev[p.id], vp: (prev[p.id].vp || 0) + bonus } }));
              addLog("🎸 Year End", `${pName}: ${a.name} → +${bonus} VP (highest fame${isHighestTickets ? " + most tickets" : ""})`);
              showFloatingBonus(`+${bonus} ⭐ ${a.name}`, "#c4b5fd");
            }
          }
          // Daft Punk: +1 VP / 3 Amenities
          if (eff.includes("vp / 3 amenities")) {
            const amVP = Math.floor((pd.amenities || []).length / 3);
            if (amVP > 0) {
              setPlayerData(prev => ({ ...prev, [p.id]: { ...prev[p.id], vp: (prev[p.id].vp || 0) + amVP } }));
              addLog("🎸 Year End", `${pName}: ${a.name} → +${amVP} VP (${(pd.amenities||[]).length} amenities / 3)`);
              showFloatingBonus(`+${amVP} ⭐ ${a.name}`, "#c4b5fd");
            }
          }
          // Fatboy Slim: +1 VP / active council objective
          if (eff.includes("vp / council objective")) {
            ...
            if (activeCount > 0) {
              setPlayerData(prev => ({ ...prev, [p.id]: { ...prev[p.id], vp: (prev[p.id].vp || 0) + activeCount } }));
              addLog("🎸 Year End", `${pName}: ${a.name} → +${activeCount} VP (active councils)`);
              showFloatingBonus(`+${activeCount} ⭐ ${a.name}`, "#c4b5fd");
            }
          }
          // Gorillaz: +1 VP per campsite
          if (eff.includes("1vp per existing campsite") || eff.includes("1 vp per existing campsite")) {
            ...
            if (camps > 0) {
              setPlayerData(prev => ({ ...prev, [p.id]: { ...prev[p.id], vp: (prev[p.id].vp || 0) + camps } }));
              addLog("🎸 Year End", `${pName}: ${a.name} → +${camps} VP (campsites)`);
              showFloatingBonus(`+${camps} ⭐ ${a.name}`, "#c4b5fd");
            }
          }
          // Kendrick: Year End: -X VP. Sell Y tickets
          { const vpLoss = (a.effect || "").match(/Year End:.*-(\d+)\s*VP/i);
            if (vpLoss) {
              const loss = parseInt(vpLoss[1]);
              setPlayerData(prev => ({ ...prev, [p.id]: { ...prev[p.id], vp: Math.max(0, (prev[p.id].vp || 0) - loss) } }));
              addLog("🎸 Year End", `${pName}: ${a.name} → -${loss} VP`);
              showFloatingBonus(`-${loss} ⭐ ${a.name}`, "#ef4444");
            }
          }
          { const sellTix = (a.effect || "").match(/Year End:.*[Ss]ell\s+(\d+)\s+tickets?/i);
            if (sellTix) {
              const tix = parseInt(sellTix[1]);
              setPlayerData(prev => ({ ...prev, [p.id]: { ...prev[p.id], bonusTickets: (prev[p.id].bonusTickets || 0) + tix } }));
              addLog("🎸 Year End", `${pName}: ${a.name} → +${tix} ticket sales`);
              showFloatingBonus(`+${tix} 🎟️ ${a.name}`, "#fbbf24");
            }
          }
          // Eminem: +1 VP / Hip Hop artist played this year
          if (eff.includes("vp / hip hop artist")) {
            const hhCount = (pd.stageArtists || []).flat().filter(ba => ba.genre && ba.genre.includes("Hip Hop")).length;
            if (hhCount > 0) {
              setPlayerData(prev => ({ ...prev, [p.id]: { ...prev[p.id], vp: (prev[p.id].vp || 0) + hhCount } }));
              addLog("🎸 Year End", `${pName}: ${a.name} → +${hhCount} VP (${hhCount} Hip Hop artists)`);
              showFloatingBonus(`+${hhCount} ⭐ ${a.name}`, "#c4b5fd");
            }
          }
        } catch(err) { /* skip */ }
      }));
          ...
          <div style={{ ...card, width: "100%", maxWidth: 700, marginTop: 12, padding: 16, alignSelf: "center" }}>
            ...
            {!actionTaken && turnAction === "artist" && artistAction === null && <div style={{ textAlign: "center" }}>
              ...
              <div style={{ display: "flex", gap: 8, justifyContent: "center", flexWrap: "wrap" }}>
                ...
                <button onClick={() => setArtistAction("reservePool")} disabled={artistPool.length === 0} style={{ ...bs, fontSize: 12, opacity: artistPool.length === 0 ? 0.4 : 1 }}>📥 Pick up from Pool</button>
                <button onClick={() => handleReserveFromDeck()} style={{ ...bs, fontSize: 12 }}>📦 Draw from Deck</button>
              </div>
              {(currentPD.amenities || []).some(a => a.type === "portaloo") && <button onClick={() => setArtistAction("spendPortaloo")} style={{ ...bs, fontSize: 11, marginTop: 10, background: "rgba(96,165,250,0.15)", border: "1px solid #60a5fa", color: "#60a5fa" }}>🚽 Spend a Portaloo to refresh the pool (×1 or ×2)</button>}
            ...
            {!actionTaken && turnAction === "artist" && artistAction === "reservePool" && <div style={{ textAlign: "center" }}>
              <p style={{ color: "#ec4899", fontSize: 13, marginBottom: 8 }}>Pick up an artist from the pool to add to your hand:</p>
              <div style={{ display: "flex", gap: 8, justifyContent: "center", flexWrap: "wrap" }}>
            ...
            {!actionTaken && turnAction === "artist" && artistAction === "deckSwapPool" && <div style={{ textAlign: "center" }}>
              ...
              <p style={{ color: "#94a3b8", fontSize: 11, marginBottom: 8 }}>Click a pool artist to discard and swap:</p>
              <div style={{ display: "flex", gap: 8, justifyContent: "center", flexWrap: "wrap" }}>
                {artistPool.map((a, i) => <div key={i} style={{ cursor: "pointer" }} onClick={() => handleDeckSwapPool(i)}>
