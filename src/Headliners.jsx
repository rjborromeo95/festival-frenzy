...
export default function Headliners() {
  ...
  function bookArtistToStage(artist, stageIdx, pid) {
    setPlayerData(prev => {
      ...
      // Guard: don't book if stage is full or artist already on ANY stage for this player
      if ((sa[stageIdx] || []).length >= 3) return prev;
      const allBooked = sa.flat().map(a => a.name);
      if (allBooked.includes(artist.name)) { console.warn("Duplicate artist blocked:", artist.name); return prev; }
  ...
  const beginRoundEnd = () => {
    ...
    for (const p of players) {
      ...
      // Ticket VP: +1 VP to player with most tickets (split if tied)
      ...
      nat[p.id][year] = { raw: rawT, fame: finalFame, fameVP: finalFameVP, ticketVP };
        ...
        {!isMobile ? <div style={{ width: 220, padding: 16, borderRight: "1px solid #2a2a4a", overflowY: "auto", flexShrink: 0 }}>
          ...
          {!isMobile && <>
            ...
            {sidebarTab === "my" && <div style={{ marginTop: 6 }}>
              {playerObjectives[currentPlayerId] && (() => { const obj = playerObjectives[currentPlayerId]; const { count } = countGenreLineups(obj, currentPD); return <div style={{ padding: 8, borderRadius: 8, background: "rgba(124,58,237,0.1)", border: "1px solid rgba(124,58,237,0.3)", marginBottom: 6 }}>
                <div style={{ fontSize: 9, fontWeight: 700, color: "#c4b5fd", textTransform: "uppercase" }}>🎯 Artist Objective</div>
                <div style={{ fontSize: 10, fontWeight: 700, color: "#e9d5ff" }}>{obj.name}</div>
                <div style={{ fontSize: 9, color: "#94a3b8", marginTop: 2 }}>{obj.req}</div>
                {count > 0 && <div style={{ fontSize: 9, color: "#4ade80", marginTop: 2 }}>✅ {count} qualifying lineup{count > 1 ? "s" : ""}</div>}
                <div style={{ fontSize: 9, color: "#4ade80", marginTop: 3 }}>1st: {obj.reward1}</div>
                <div style={{ fontSize: 9, color: "#fbbf24" }}>2nd: {obj.reward2}</div>
              </div>; })()}
