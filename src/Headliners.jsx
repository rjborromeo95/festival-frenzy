...
const ALL_GOALS = [
  ...
  { id: "marketing_gimmicks", name: "Marketing Gimmicks", trackKey: "fameDieRolls",
    ...
    req3: "Gain Fame from the amenity dice 4 times", req3Target: 4 },
  { id: "its_a_sign", name: "It's a Sign", trackKey: "artistsSigned",
    ...
    req2: "Sign an artist to hand 2 times", req2Target: 2,
    req3: "Sign an artist to hand 4 times", req3Target: 4 },
  { id: "government_catering", name: "Government Catering", trackKey: "councilsBought",
    ...
    req2: "Buy a council objective with catering 2 times", req2Target: 2,
    req3: "Buy a council objective with catering 3 times", req3Target: 3 },
...
export default function Headliners() {
  ...
  function trackGoalProgress(pid, trackKey) {
    setGoalProgress(prev => {
      ...
      // Check each active goal — use functional updater to prevent stale reads
      activeGoals.forEach((ag, gi) => {
        ...
        if (newVal >= ag.goal.req1Target) {
          setGoalReq1Claimed(prev2 => {
            if (prev2[ag.goal.id]?.[pid]) return prev2;
        ...
        // Req 2 — first player only (use ref for sync check)
        if (newVal >= ag.goal.req2Target && goalClaimsRef.current[`${ag.goal.id}_req2`] === undefined) {
          goalClaimsRef.current[`${ag.goal.id}_req2`] = pid;
        ...
        if (newVal >= ag.goal.req3Target && goalClaimsRef.current[`${ag.goal.id}_req3`] === undefined) {
          goalClaimsRef.current[`${ag.goal.id}_req3`] = pid;
  ...
  const proceedFromRoundEnd = () => {
    ...
    // ALL players go through pre-round (stage opening + free amenities)
          ...
          <div style={{ ...card, width: "100%", maxWidth: 700, marginTop: 12, padding: 16, alignSelf: "center" }}>
            ...
            {!actionTaken && turnAction === "artist" && artistAction === "spendPortaloo" && <div style={{ textAlign: "center" }}>
              ...
              <HexGrid stages={currentPD.stages || []} amenities={currentPD.amenities || []} stageColors={currentPD.stageColors || []} onHexClick={(col, row) => {
                ...
                sfx.placeAmenity();
                refreshPool();
                // Second refresh after a tick so the pool refills first
                setTimeout(() => { refreshPool(); recalcTickets(); }, 50);
