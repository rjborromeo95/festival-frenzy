...
export default function Headliners() {
  ...
  function bookArtistToStage(artist, stageIdx, pid) {
    setPlayerData(prev => {
      ...
      pd.stageArtists = sa;
      // Award the artist's VP immediately
      pd.vp = (pd.vp || 0) + (artist.vp || 0);
    ...
    if (artist.vp > 0) { showFloatingBonus(`+${artist.vp} ⭐`, "#c4b5fd"); sfx.gainVP(); }
  ...
  const startGame = () => {
    ...
    const tcPool = ALL_COUNCIL_OBJECTIVES.filter(c => !Object.values(pCouncils).flat().some(pc => pc.obj.id === c.id));
  ...
  const aiStep = useCallback(() => {
    ...
    if (pendingEffect && pendingEffectPid !== null) {
      ...
      if (pe.type === "placeSpecific" || (pe.type === "placeAmenity" && pe.chosenType)) {
        ...
        setPlayerData(p => ({ ...p, [pid]: { ...p[pid], amenities: [...p[pid].amenities, { col: pos.col, row: pos.row, type: aType }] } }));
        addLog("🤖 AI", `Placed bonus ${AMENITY_LABELS[aType]}`);
        checkSecurityVPBonus(pid, aType);
    ...
    // ─── GAME PHASE ───
    if (phase === "game") {
      if (showYearAnnouncement) { setShowYearAnnouncement(false); scheduleNext(500); return; }
      ...
      if (decision.action === "book") {
        ...
        if (source === "pool") handleBookFromPool(artistIdx);
        else handleBookFromHand(artistIdx);
        // handleStageSelect needs to fire after state update
        aiProcessing.current = false;
        setTimeout(() => {
          handleStageSelect(stageIdx);
          // After booking, effects may trigger — schedule next step for endTurn or pending effects
          aiTimer.current = setTimeout(() => aiStep(), 600);
        }, 400);
        return;
      }
  ...
  }, [phase, setupStep, setupIndex, currentPlayerIdx, showTurnStart, actionTaken, noTurnsLeft, pendingEffect, showHeadliner, showCouncilFame]);
  ...
  const beginRoundEnd = () => {
    ...
    setPlayerData(prev => {
      ...
      for (const p of players) {
        ...
        if (trendingObjective) {
          const tRes = evaluateObjective(trendingObjective, pd, true);
          if (tRes === -999) {
            next[p.id] = { ...next[p.id], baseFame: Math.min(FAME_MAX, (next[p.id].baseFame || 0) + 2) };
            addLog(p.festivalName, `📢 Achieved Trending "${trendingObjective.name}" → +2 🔥`);
          }
        }
    ...
    return (<div style={CS}>{logBtn}{discardBtn}
      ...
      {showHeadliner && <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.9)", zIndex: 950, display: "flex", alignItems: "center", justifyContent: "center" }} onClick={() => setShowHeadliner(null)}>
        <div style={{ textAlign: "center", animation: "bookReveal 0.5s" }}>
          ...
          {showHeadliner.artist.effect && <p style={{ color: "#fbbf24", fontSize: 14, padding: "8px 16px", borderRadius: 10, background: "rgba(251,191,36,0.1)", display: "inline-block" }}>✨ {showHeadliner.artist.effect} <span style={{ color: "#f87171" }}>(×2!)</span></p>}
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 16 }}>
              {sa.map((a, ai) => {
                ...
                return <div key={ai} style={{ padding: 14, borderRadius: 14, background: genreGradient(a.genre), color: "#fff", textAlign: "left", position: "relative", border: isHL ? "3px solid #fbbf24" : "2px solid rgba(255,255,255,0.1)", boxShadow: isHL ? "0 0 20px rgba(251,191,36,0.3)" : "0 2px 8px rgba(0,0,0,0.3)" }}>
                  ...
                  {a.effect && <div style={{ fontSize: 11, marginTop: 8, padding: "5px 10px", borderRadius: 8, background: "rgba(0,0,0,0.3)", fontStyle: "italic" }}>✨ {a.effect}{isHL ? " (triggers ×2)" : ""}</div>}
                </div>;
              })}
            </div>
      ...
      {pendingEffect && pendingEffectPid === currentPlayerId && (() => {
        ...
        const handleEffectPlacement = (col, row) => {
          ...
          const aType = pe.amenityType || pe.chosenType;
          if (!aType) return;
          setPlayerData(p => ({ ...p, [pid]: { ...p[pid], amenities: [...p[pid].amenities, { col, row, type: aType }] } }));
          addLog("Effect", `Placed bonus ${AMENITY_LABELS[aType]} at (${col},${row})`);
          checkSecurityVPBonus(pid, aType);
          sfx.placeAmenity();
          const remaining = (pe.placeCount || 1) - 1;
          if (remaining > 0) {
            // More placements needed — re-queue with decremented count, reset chosenType for placeAmenity
            if (pe.type === "placeAmenity") {
              setPendingEffect({ ...pe, placeCount: remaining, chosenType: null });
            } else {
              setPendingEffect({ ...pe, placeCount: remaining });
            }
          } else {
            setPendingEffect(null); setPendingEffectPid(null);
          }
      ...
      {showYearAnnouncement && <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.9)", zIndex: 910, display: "flex", alignItems: "center", justifyContent: "center" }} onClick={() => setShowYearAnnouncement(false)}>
        <div style={{ ...card, textAlign: "center", maxWidth: 500, width: "100%", animation: "fadeSlideIn 0.4s" }}>
          ...
          {trendingObjective && <div style={{ padding: 12, borderRadius: 10, background: "rgba(251,191,36,0.1)", border: "1px solid rgba(251,191,36,0.3)", marginBottom: 10, textAlign: "left" }}>
            <div style={{ fontSize: 10, fontWeight: 700, color: "#fbbf24", textTransform: "uppercase", letterSpacing: 1 }}>📢 Trending Artist Objective</div>
            <div style={{ fontSize: 14, fontWeight: 700, color: "#e9d5ff", marginTop: 4 }}>{trendingObjective.name}</div>
            <div style={{ fontSize: 11, color: "#94a3b8", marginTop: 2 }}>{trendingObjective.req}</div>
            <div style={{ fontSize: 11, color: "#fbbf24", marginTop: 4 }}>🔥 Achieve → +2 Fame</div>
          </div>}
          ...
          <p style={{ color: "#6b7280", fontSize: 11, marginTop: 12 }}>Click anywhere to start playing</p>
        </div>
      </div>}
      {/* Turn start popup */}
      {showTurnStart && <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", zIndex: 900, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ ...card, textAlign: "center", maxWidth: 440, animation: "fadeSlideIn 0.3s" }}>
          ...
          {trendingObjective && <div style={{ marginTop: 12, padding: 10, borderRadius: 10, background: "rgba(251,191,36,0.08)", border: "1px solid rgba(251,191,36,0.25)" }}>
            ...
            <div style={{ fontSize: 11, color: "#94a3b8", marginTop: 2 }}>{trendingObjective.req}</div>
            <div style={{ fontSize: 10, color: "#fbbf24", marginTop: 4 }}>🔥 Achieve → +2 Fame</div>
          </div>}
      ...
      <div style={{ display: "flex", minHeight: "100vh" }}>
        ...
        <div style={{ width: 220, padding: 16, borderRight: "1px solid #2a2a4a", overflowY: "auto", flexShrink: 0 }}>
          ...
          {sidebarTab === "trending" && <>
            {/* Trending Artist Objective */}
            {trendingObjective && <div style={{ marginTop: 8, padding: 10, borderRadius: 10, background: "rgba(251,191,36,0.1)", border: "1px solid rgba(251,191,36,0.3)", cursor: "help" }} className="obj-hover-parent">
              <div style={{ fontSize: 10, fontWeight: 700, color: "#fbbf24", marginBottom: 4, textTransform: "uppercase", letterSpacing: 1 }}>📢 Trending Artist Objective</div>
              <div style={{ fontSize: 11, fontWeight: 700, color: "#e9d5ff" }}>{trendingObjective.name}</div>
              <div style={{ fontSize: 10, color: "#94a3b8", marginTop: 2 }}>{trendingObjective.req}</div>
              <div style={{ fontSize: 10, color: "#fbbf24", marginTop: 2 }}>🔥 Achieve → +2 Fame</div>
              <div className="obj-hover-tip" style={{ position: "relative", width: "100%", padding: 0, borderRadius: 10, maxHeight: 0, overflow: "hidden", transition: "all 0.2s", background: "#1a1a2e", border: "1px solid #fbbf24", boxShadow: "0 4px 20px rgba(0,0,0,0.5)", textAlign: "left" }}>
                ...
                <div style={{ fontSize: 10, color: "#fbbf24", fontWeight: 600 }}>🔥 Achieve this on any stage → +2 Fame</div>
              </div>
            </div>}
        ...
        <div style={{ ...card, textAlign: "center", maxWidth: 600, width: "100%" }}>
          ...
          {evtRes && <>
            <div style={{ marginBottom: 12, padding: 8, borderRadius: 10, background: "rgba(124,58,237,0.1)", fontSize: 12, color: "#94a3b8" }}>
              👮‍♀️ Security: {evtRes.secCount} • 🔴 Negative events: {evtRes.totalNeg} • 🛡️ Blocked: {evtRes.blocked.length}
            </div>
