...
const ALL_COUNCIL_OBJECTIVES = [
  ...
  { id: "form_a_line", name: "Form a Line", flavour: "The council want an orderly food area.", req: "Catering vans in a single line", benefit: "+1 ticket/catering in line (min 2)", tBenefit: "+1 Fame when declining stage" },
...
function evalCouncilObjective(obj, pd, isTrending) {
  ...
  switch (obj.id) {
    ...
    case "chef_beef": count = ofType("catering").filter(c => ofType("catering").every(o => o === c || hexDistance(c.col, c.row, o.col, o.row) >= 5)).length; break;
  ...
  switch (obj.id) {
    ...
    case "form_a_line": r.tickets = count; break;
...
export default function Headliners() {
  ...
  const startSetup = () => {
    ...
    const goals = selectedGoals.map((goal, gi) => {
      const f3 = fame3Artists[gi] || fame3Artists[0];
      const f5 = fame5Artists[gi] || fame5Artists[0];
      return { goal, setAsideArtists: [f3, f5], req2ClaimedBy: null, req3ClaimedBy: null };
    });
    setActiveGoals(goals);
    // Remove set-aside artists from the draft pools
  ...
  const beginRoundEnd = () => {
    ...
    // Artist objectives now apply at round start via applyObjectiveRewards(), not at year end
        ...
        setPlayerData(prev => {
          ...
          for (const p of players) {
            ...
            (pd.stageArtists || []).forEach((sa, si) => sa.forEach((a, ai) => {
              ...
              // Coldplay: +1 VP / Fame gained this year (baseFame tracks this)
              if (eff.includes("vp / fame gained") || (eff.includes("vp / fame") && !eff.includes("highest"))) {
              ...
              if (eff.includes("vp if you have the highest fame")) {
                ...
                const isHighest = players.every(op => op.id === p.id || (next[op.id]?.fame || 0) <= myFame);
                if (isHighest) {
                  vpBonus += 1;
                  const myTickets = pd.tickets || 0;
                  const mostTickets = players.every(op => op.id === p.id || (next[op.id]?.tickets || 0) <= myTickets);
              ...
              // Kendrick: +1 VP / Negative Event avoided
              ...
              // Eminem: +1 VP / Negative Event that hit you
              ...
              // Fatboy Slim: +1 VP / Council Objective giving benefit
  ...
  const startNextYear = () => {
    ...
    setTrendingCouncil(tc);
    addLog("📋 Trending Council", `${tc.name} — ${tc.tBenefit}`);
    // Apply artist objective rewards from last year's lineups
            ...
            {activeGoals.map((ag, gi) => {
              ...
              return <div key={gi} style={{ marginTop: 8, padding: 10, borderRadius: 10, background: "rgba(251,191,36,0.06)", border: "1px solid rgba(251,191,36,0.2)" }}>
                ...
                <div style={{ padding: 6, borderRadius: 6, marginBottom: 4, background: ag.req2ClaimedBy !== null ? "rgba(107,114,128,0.15)" : "rgba(251,191,36,0.08)", border: `1px solid ${ag.req2ClaimedBy !== null ? "#4b556340" : "#fbbf2440"}`, opacity: ag.req2ClaimedBy !== null ? 0.6 : 1 }}>
                  ...
                  <div style={{ fontSize: 9, color: "#94a3b8" }}>Prize: <strong style={{ color: "#e9d5ff" }}>{ag.setAsideArtists[0].name}</strong> (★{ag.setAsideArtists[0].fame} — plays free)</div>
                </div>
                <div style={{ padding: 6, borderRadius: 6, background: ag.req3ClaimedBy !== null ? "rgba(107,114,128,0.15)" : "rgba(251,191,36,0.08)", border: `1px solid ${ag.req3ClaimedBy !== null ? "#4b556340" : "#fbbf2440"}`, opacity: ag.req3ClaimedBy !== null ? 0.6 : 1 }}>
                  ...
                  <div style={{ fontSize: 9, color: "#94a3b8" }}>Prize: <strong style={{ color: "#fbbf24" }}>{ag.setAsideArtists[1].name}</strong> (★{ag.setAsideArtists[1].fame} — plays free)</div>
                </div>
                <div style={{ marginTop: 6, fontSize: 9, color: "#64748b" }}>
                  {players.map(p => <span key={p.id} style={{ marginRight: 8, color: p.id === currentPlayerId ? "#e9d5ff" : "#64748b" }}>{p.festivalName}: {goalProgress[p.id]?.[g.trackKey] || 0}</span>)}
                </div>
              </div>;
            })}
