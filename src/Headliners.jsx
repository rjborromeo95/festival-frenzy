...
export default function Headliners() {
  ...
  const offerObjectiveChoice = (pid) => {
    // Don't offer if there's already a pending choice
    if (pendingObjectiveChoice && pendingObjectiveChoice.options.length > 0) return;
    setObjectiveDeck(prev => {
      const deck = [...prev];
      if (deck.length < 2) return prev; // not enough objectives left
  ...
  const endTurn = () => {
    ...
    // Check if the NEXT player needs a new objective choice
    const nextPid = turnOrder[ni];
    const nextObjs = playerObjectives[nextPid] || [];
    const nextHasUncompleted = nextObjs.some(e => !e.completed);
    const alreadyPending = pendingObjectiveChoice && pendingObjectiveChoice.options.length >= 2;
    if (!alreadyPending && !nextHasUncompleted && nextObjs.length > 0 && objectiveDeck.length >= 2) {
      offerObjectiveChoice(nextPid);
    } else if (!alreadyPending && pendingObjectiveChoice?.needsDraw && pendingObjectiveChoice?.playerId === nextPid && objectiveDeck.length >= 2) {
      offerObjectiveChoice(nextPid);
    }
