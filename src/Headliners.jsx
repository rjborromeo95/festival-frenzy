        ...
        <div style={{ width: 220, padding: 16, borderRight: "1px solid #2a2a4a", overflowY: "auto", flexShrink: 0 }}>
          ...
          {trendingObjective && <div style={{ marginTop: 10, padding: 10, borderRadius: 10, background: "rgba(251,191,36,0.1)", border: "1px solid rgba(251,191,36,0.3)", position: "relative", cursor: "help" }} className="obj-hover-parent">
            ...
            <div className="obj-hover-tip" style={{ position: "relative", width: "100%", padding: 0, borderRadius: 10, maxHeight: 0, overflow: "hidden", transition: "all 0.2s", background: "#1a1a2e", border: "1px solid #fbbf24", boxShadow: "0 4px 20px rgba(0,0,0,0.5)", textAlign: "left" }}>
              ...
              <div style={{ fontSize: 10, color: "#fbbf24", fontWeight: 600, marginTop: 4 }}>🔥 Achieve this on any stage → +2 Fame</div>
            </div>
          </div>}
          {/* Personal Objective */}
          ...
          {trendingCouncil && (() => {
            ...
            return <div style={{ marginTop: 8, padding: 8, borderRadius: 8, background: tcRes.active ? "rgba(34,197,94,0.1)" : "rgba(124,58,237,0.06)", border: `1px solid ${tcRes.active ? "#22c55e40" : "#4c1d9540"}`, cursor: "help" }} className="obj-hover-parent">
              <div style={{ fontSize: 10, fontWeight: 700, color: "#22c55e", textTransform: "uppercase", letterSpacing: 1 }}>📋📢 Trending Council</div>
              <div style={{ fontSize: 10, fontWeight: 600, color: tcRes.active ? "#4ade80" : "#c4b5fd" }}>{tcRes.active ? "✅" : "❌"} {trendingCouncil.name}</div>
              <div style={{ fontSize: 9, color: "#fbbf24" }}>{trendingCouncil.tBenefit}</div>
              {tcRes.active && tcRes.fame > 0 && <div style={{ fontSize: 9, color: "#4ade80", marginTop: 2 }}>+{tcRes.fame} 🔥 Fame active</div>}
              <div className="obj-hover-tip" style={{ position: "relative", width: "100%", padding: 0, borderRadius: 10, maxHeight: 0, overflow: "hidden", transition: "all 0.2s", background: "#1a1a2e", border: "1px solid #22c55e", boxShadow: "0 4px 20px rgba(0,0,0,0.5)", textAlign: "left" }}>
                ...
                <div style={{ fontSize: 10, color: "#c4b5fd", marginBottom: 4 }}>📌 Requirement: {trendingCouncil.req}</div>
                <div style={{ fontSize: 10, color: "#fbbf24" }}>Trending: {trendingCouncil.tBenefit}</div>
                <div style={{ fontSize: 10, color: "#94a3b8", marginTop: 2 }}>All players can benefit from this!</div>
              </div>
            </div>;
          })()}
          {/* Fame breakdown */}
          ...
          {/* Microtrends */}
          {microtrends.length > 0 && <div style={{ marginTop: 8, padding: 8, borderRadius: 8, background: "rgba(124,58,237,0.06)" }}>
            <div style={{ fontSize: 10, fontWeight: 700, color: "#e9d5ff", marginBottom: 4, textTransform: "uppercase", letterSpacing: 1 }}>🎵 Microtrends</div>
            {microtrends.map((mt, i) => {
              ...
              return <div key={i} style={{ padding: "4px 8px", borderRadius: 6, marginBottom: 3, background: claimed ? "rgba(107,114,128,0.15)" : `${GENRE_COLORS[mt.genre]}15`, border: `1px solid ${claimed ? "#4b5563" : GENRE_COLORS[mt.genre]}40`, opacity: claimed ? 0.5 : 1 }}>
                <div style={{ fontSize: 10, fontWeight: 700, color: claimed ? "#6b7280" : GENRE_COLORS[mt.genre] }}>
