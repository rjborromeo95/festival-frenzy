...
const ALL_EVENTS = [
  ...
  // Negative events — all avoidable with security (-3 VP each)
  { name: "Blocked Toilets", color: "red", desc: "One of your portaloos backs up.", cond: "has_more_than_1_portaloo", result: "-3 VP", avoidable: true, apply: () => ({ vp: -3 }) },
  { name: "Food Poisoning", color: "red", desc: "Your food vans served medium rare chicken.", cond: null, result: "-3 VP", avoidable: true, apply: () => ({ vp: -3 }) },
  { name: "Powerful Gusts", color: "red", desc: "Campsites on a hill get battered.", cond: "campsite_above_stage", result: "-3 VP", avoidable: true, apply: () => ({ vp: -3 }) },
  { name: "Flooding", color: "red", desc: "Campers in the valley wake up in a bog.", cond: "campsite_below_stage", result: "-3 VP", avoidable: true, apply: () => ({ vp: -3 }) },
  { name: "Speakers on Fire", color: "red", desc: "A speaker stops working during a set.", cond: null, result: "-3 VP", avoidable: true, apply: () => ({ vp: -3 }) },
  { name: "Thieves Among Us", color: "red", desc: "Campers complain about lost earrings.", cond: "campsites_gt_security", result: "-3 VP", avoidable: true, apply: () => ({ vp: -3 }) },
  { name: "TED Talk on Stage", color: "red", desc: "A headliner uses their set for politics.", cond: "has_headliner", result: "-3 VP", avoidable: true, apply: () => ({ vp: -3 }) },
  { name: "Rowdy Crowd", color: "red", desc: "A fight erupted in the crowd.", cond: null, result: "-3 VP", avoidable: true, apply: () => ({ vp: -3 }) },
  { name: "Passed Out", color: "red", desc: "Someone fainted in the crowd.", cond: "less_than_2_security", result: "-3 VP", avoidable: true, apply: () => ({ vp: -3 }) },
  { name: "Dehydration", color: "red", desc: "Long water queues.", cond: "has_catering", result: "-3 VP", avoidable: true, apply: () => ({ vp: -3 }) },
  { name: "Shakey Ground", color: "red", desc: "Some campsites weren't built on soft enough soil.", cond: "has_campsite", result: "-3 VP", avoidable: true, apply: () => ({ vp: -3 }) },
  { name: "Dodgy Dealings", color: "red", desc: "Some security guards weren't here to work.", cond: "has_security", result: "-3 VP", avoidable: true, apply: () => ({ vp: -3 }) },
  { name: "Broken Walkie-Talkies", color: "red", desc: "Security guards couldn't hear each other.", cond: "has_security", result: "-3 VP", avoidable: true, apply: () => ({ vp: -3 }) },
  { name: "Desperate", color: "red", desc: "Festival goers couldn't make it to the toilet in time.", cond: "has_portaloo", result: "-3 VP", avoidable: true, apply: () => ({ vp: -3 }) },
  { name: "Long Queues", color: "red", desc: "Demand was too high for catering.", cond: "has_catering", result: "-3 VP", avoidable: true, apply: () => ({ vp: -3 }) },
  { name: "If You Gotta Go...", color: "red", desc: "Demand was too high for portaloos.", cond: "has_portaloo", result: "-3 VP", avoidable: true, apply: () => ({ vp: -3 }) },
  { name: "Overstocking", color: "red", desc: "Catering companies complained about sales.", cond: "has_catering", result: "-3 VP", avoidable: true, apply: () => ({ vp: -3 }) },
];
...
function aiDecideTurn(pd, artistPool, dice, year) {
  ...
  const bookableHand = (pd.hand || []).filter(a => !bookedNames.has(a.name) && counts.campsite >= a.campCost && counts.security >= a.securityCost && counts.catering >= a.cateringCost && counts.portaloo >= a.portalooCost);
...
export default function Headliners() {
  ...
  const [objectiveDeck, setObjectiveDeck] = useState([]); // remaining objectives not yet dealt
  ...
  function bookArtistToStage(artist, stageIdx, pid) {
    ...
    // Check artist objectives on lineup completion
    if (isHeadliner) {
      setTimeout(() => {
        const updatedPd = playerData[pid]; // may be stale — use latest via callback
  ...
  const offerObjectiveChoice = (pid) => {
    setObjectiveDeck(prev => {
      ...
      if (opt1 && opt2) setPendingObjectiveChoice({ playerId: pid, options: [opt1, opt2] });
      else if (opt1) setPendingObjectiveChoice({ playerId: pid, options: [opt1] });
      return deck;
    });
  };
  ...
  const startGame = () => {
    ...
    // Draw trending council objective — exclude multi-stage if any player has 1 stage
    const anyOneStage = players.some(p => (playerData[p.id]?.stages || [{ col: 0, row: 0 }]).length <= 1);
    const tcPool = ALL_COUNCIL_OBJECTIVES.filter(c => !Object.values(playerCouncils).flat().some(pc => pc.obj?.id === c.id) && !(anyOneStage && c.requiresMultiStage));

    // Offer first player their objective choice
    offerObjectiveChoice(order[0]);
  ...
  const aiStep = () => {
    ...
    if (phase === "game") {
      ...
      if (showTurnStart) {
        // AI: auto-choose pending objective
        if (pendingObjectiveChoice && pendingObjectiveChoice.options.length >= 2 && pendingObjectiveChoice.playerId === currentPlayerId) {
          const pick = pendingObjectiveChoice.options[Math.floor(Math.random() * pendingObjectiveChoice.options.length)];
          chooseObjective(pick);
          scheduleNext(300); return;
  ...
  const endTurn = () => {
    ...
    // Check if the NEXT player needs a new objective choice (completed one on a previous turn)
    const nextPid = turnOrder[ni];
    const nextObjs = playerObjectives[nextPid] || [];
    const nextHasUncompleted = nextObjs.some(e => !e.completed);
    if (!nextHasUncompleted && nextObjs.length > 0 && objectiveDeck.length >= 2) {
      // All their objectives are completed — offer new pair
      offerObjectiveChoice(nextPid);
    } else if (pendingObjectiveChoice?.needsDraw && pendingObjectiveChoice?.playerId === nextPid) {
      offerObjectiveChoice(nextPid);
    }
  ...
  const beginRoundEnd = () => {
    ...
    for (const p of players) {
      ...
      // Council VP
      let councilVP = 0;
      (playerCouncils[p.id] || []).forEach(co => { if (co.active) { try { councilVP += calcCouncilBenefit(co.obj, pd, false).vp; } catch(e){} } });
      vpBonus += councilVP;
      ...
      (pd.stageArtists || []).forEach(sa => sa.forEach(a => {
        try {
          const eff = (a.effect || "").toLowerCase();
          if (eff.includes("vp / negative event avoided")) effectVP += eventPhaseResults?.[p.id]?.blocked?.length || 0;
          if (eff.includes("vp / negative event that hit")) effectVP += eventPhaseResults?.[p.id]?.negative?.length || 0;
        } catch(err) { /* skip */ }
      }));
      vpBonus += effectVP;
      ...
      nat[p.id][year] = { raw: rawT, fame: finalFame, fameVP: finalFameVP, ticketVP, artistVP, councilVP, effectVP, preYearVP, totalYearVP: vpBonus };
  ...
  const startNextYear = () => {
    ...
    setMicrotrends(mt);
    addLog("🎵 Microtrends", `Book a ${mt[0].genre} artist • Book a ${mt[1].genre} artist`);
    // Draw new trending council — exclude multi-stage if any player has 1 stage
    const anyOneStageNow = players.some(p => (playerData[p.id]?.stages || []).length <= 1);
    const tcPool = ALL_COUNCIL_OBJECTIVES.filter(c => c.id !== trendingCouncil?.id && !(anyOneStageNow && c.requiresMultiStage));
  ...
  if (phase === "objectiveChoice" && pendingObjectiveChoice) {
    ...
    // Auto-choose for AI
    if (isAI && pendingObjectiveChoice.options.length > 0) {
      setTimeout(() => {
        const pick = pendingObjectiveChoice.options[Math.floor(Math.random() * pendingObjectiveChoice.options.length)];
        chooseObjective(pick);
        // Advance to next player or start game
        const allChosen = players.every(p => p.id === pendingObjectiveChoice.playerId || (playerObjectives[p.id] || []).length > 0);
        const nextP = players.find(p => (playerObjectives[p.id] || []).length === 0 && p.id !== pendingObjectiveChoice.playerId);
        if (nextP) offerObjectiveChoice(nextP.id);
        else setTimeout(() => setPhase("game"), 200);
      }, 500);
    }
    ...
    return (<div style={CS}>{utilButtons}
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", minHeight: "100vh", padding: 24 }}>
        <div style={{ ...card, textAlign: "center", maxWidth: 550, width: "100%" }}>
          ...
          {/* Show hand */}
          <div style={{ marginBottom: 16, padding: 10, borderRadius: 10, background: "rgba(124,58,237,0.08)", border: "1px solid #7c3aed30" }}>
          ...
          <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap" }}>
            {pendingObjectiveChoice.options.map((obj, i) => (
              <div key={i} onClick={() => {
                ...
                chooseObjective(obj);
                const nextP = players.find(p => (playerObjectives[p.id] || []).length === 0 && p.id !== pendingObjectiveChoice.playerId);
                if (nextP) setTimeout(() => offerObjectiveChoice(nextP.id), 200);
                else setTimeout(() => setPhase("game"), 200);
  ...
  if (phase === "game") {
    ...
    return (<div style={CS}>{utilButtons}{updateNotesModal}
      ...
      {pendingObjectiveChoice && pendingObjectiveChoice.options.length >= 2 && pendingObjectiveChoice.playerId === currentPlayerId && <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", zIndex: 960, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
        ...
        {!isMobile ? <div style={{ width: 220, padding: 16, borderRight: "1px solid #2a2a4a", overflowY: "auto", flexShrink: 0 }}>
          ...
          {!isMobile && <>
            <div style={{ display: "flex", gap: 4, marginTop: 10 }}>
              ...
              <button onClick={() => setSidebarTab(sidebarTab === "goals" ? null : "goals")} style={{ flex: 1, padding: "6px 0", borderRadius: 8, border: "none", background: sidebarTab === "goals" ? "rgba(251,191,36,0.3)" : "rgba(251,191,36,0.08)", color: sidebarTab === "goals" ? "#fbbf24" : "#64748b", cursor: "pointer", fontSize: 10, fontWeight: 700 }}>🏆 Goals</button>
            </div>
            {sidebarTab === "my" && <div style={{ marginTop: 6 }}>
              ...
              {(playerCouncils[currentPlayerId] || []).length > 0 && <div style={{ fontSize: 9, fontWeight: 700, color: "#22c55e", marginBottom: 4, textTransform: "uppercase" }}>📋 Council Objectives</div>}
              {(playerCouncils[currentPlayerId] || []).map((co, ci) => { const res = evalCouncilObjective(co.obj, currentPD, false); return <div key={ci} style={{ padding: 6, borderRadius: 6, marginBottom: 4, background: res.count > 0 ? "rgba(34,197,94,0.15)" : "rgba(124,58,237,0.06)", border: `1px solid ${res.count > 0 ? "#22c55e50" : "#4c1d9540"}` }}>
                <div style={{ fontSize: 10, fontWeight: 700, color: res.count > 0 ? "#4ade80" : "#94a3b8" }}>{co.obj.name}</div>
                <div style={{ fontSize: 9, color: "#94a3b8", marginTop: 1 }}>{co.obj.req}</div>
                <div style={{ fontSize: 9, color: "#c4b5fd", marginTop: 1 }}>{co.obj.benefit}</div>
                <div style={{ fontSize: 9, color: res.count > 0 ? "#4ade80" : "#64748b", marginTop: 1 }}>
                  {res.tickets > 0 ? `✅ +${res.tickets} 🎟️ (${res.count} qualifying)` : "Not yet"}
                </div>
              </div>; })}
            ...
            {sidebarTab === "trending" && <div style={{ marginTop: 6 }}>
              {trendingCouncil && (() => { const tcRes = evalCouncilObjective(trendingCouncil, currentPD, true); return <div style={{ padding: 8, borderRadius: 8, background: "rgba(34,197,94,0.08)", border: "1px solid rgba(34,197,94,0.3)", marginBottom: 6 }}>
                <div style={{ fontSize: 9, fontWeight: 700, color: "#22c55e", textTransform: "uppercase" }}>📋 Trending Council</div>
                <div style={{ fontSize: 10, fontWeight: 700, color: "#e9d5ff" }}>{trendingCouncil.name}</div>
                <div style={{ fontSize: 9, color: "#94a3b8", marginTop: 2 }}>{trendingCouncil.req}</div>
                <div style={{ fontSize: 9, color: "#fbbf24", marginTop: 1 }}>{trendingCouncil.tBenefit}</div>
                {tcRes.fame > 0 && <div style={{ fontSize: 9, color: "#4ade80", fontWeight: 600, marginTop: 2 }}>+{tcRes.fame} 🔥 Fame</div>}
              </div>; })()}
            ...
            {sidebarTab === "goals" && <div style={{ marginTop: 6 }}>
              {activeGoals.map((ag, gi) => { const g = ag.goal; const myProgress = goalProgress[currentPlayerId]?.[g.trackKey] || 0; const done = myProgress >= g.target; return <div key={gi} style={{ padding: 6, borderRadius: 6, marginBottom: 6, background: "rgba(251,191,36,0.06)", border: "1px solid rgba(251,191,36,0.2)" }}>
                <div style={{ fontSize: 10, fontWeight: 700, color: "#fbbf24" }}>🏆 {g.name} <span style={{ fontSize: 8, color: "#94a3b8", fontWeight: 400 }}>({ag.rewardType === "artist" ? "Artist" : "VP"})</span></div>
                <div style={{ fontSize: 9, color: done ? "#4ade80" : "#c4b5fd" }}>{done ? "✅" : "🎯"} {g.desc} ({myProgress}/{g.target})</div>
                {ag.rewardType === "artist" && ag.setAsideArtists[0] && <div style={{ fontSize: 8, color: "#94a3b8" }}>Prize: {ag.setAsideArtists[0].name} {ag.claimedBy1st !== null && <span style={{ color: "#4ade80" }}>→ {players.find(p => p.id === ag.claimedBy1st)?.festivalName}</span>}</div>}
                {ag.rewardType === "vp" && <div style={{ fontSize: 8, color: "#94a3b8" }}>1st: +10VP {ag.claimedBy1st !== null && <span style={{ color: "#4ade80" }}>→ {players.find(p => p.id === ag.claimedBy1st)?.festivalName}</span>} | 2nd: +5VP {ag.claimedBy2nd !== null && <span style={{ color: "#4ade80" }}>→ {players.find(p => p.id === ag.claimedBy2nd)?.festivalName}</span>}</div>}
                <div style={{ marginTop: 4 }}>
                  {players.map(p => { const prog = goalProgress[p.id]?.[g.trackKey] || 0; const pct = Math.min(100, (prog / g.target) * 100); const isMe = p.id === currentPlayerId; return <div key={p.id} style={{ marginBottom: 2 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", fontSize: 8, color: isMe ? "#e9d5ff" : "#64748b" }}>
                      <span style={{ fontWeight: isMe ? 700 : 400 }}>{p.festivalName}</span><span>{prog}/{g.target}</span>
                    </div>
                    <div style={{ height: 3, borderRadius: 2, background: "rgba(124,58,237,0.2)", overflow: "hidden" }}>
                      <div style={{ height: "100%", width: `${pct}%`, borderRadius: 2, background: isMe ? "#fbbf24" : "#7c3aed" }} />
                    </div>
                  </div>; })}
                </div>
              </div>; })}
              {activeGoals.length === 0 && <p style={{ color: "#64748b", fontSize: 10 }}>No goals.</p>}
            </div>}
          </>}
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", padding: isMobile ? "12px 16px" : 16, overflow: "auto" }}>
          ...
          <div style={{ ...card, width: "100%", maxWidth: 700, marginTop: 12, padding: 16, alignSelf: "center" }}>
            ...
            {!actionTaken && turnAction === "artist" && (artistAction === null || artistAction === "bookHand" || artistAction === "draw2") && !selectedArtist && <div style={{ textAlign: "center" }}>
              ...
              {(currentPD.amenities || []).some(a => a.type === "portaloo") && draw2Picks.length < 2 && <button onClick={() => {
                // Sacrifice a portaloo and refresh the pool — stays in draw flow
                const portaloos = (currentPD.amenities || []).filter(a => a.type === "portaloo");
                if (portaloos.length === 0) return;
                const sacrifice = portaloos[0];
                setPlayerData(p => ({ ...p, [currentPlayerId]: { ...p[currentPlayerId], amenities: p[currentPlayerId].amenities.filter(a => !(a.col === sacrifice.col && a.row === sacrifice.row && a.type === "portaloo")) } }));
                addLog(currentPlayer.festivalName, `Sacrificed 🚽 portaloo to refresh pool`);
                sfx.placeAmenity();
                refreshPool(1);
                trackGoalProgress(currentPlayerId, "portalooRefreshes");
                setTimeout(() => recalcTickets(), 50);
              }} style={{ ...bs, fontSize: 12, marginTop: 6, background: "rgba(96,165,250,0.15)", border: "1px solid #60a5fa", color: "#60a5fa" }}>🚽 Refresh Pool ({(currentPD.amenities || []).filter(a => a.type === "portaloo").length} left)</button>}
