...
export default function Headliners() {
  ...
  const recalcTickets = useCallback(() => {
    setPlayerData(prev => {
      ...
      for (const pid of Object.keys(next)) {
        ...
        // Compute fame directly (using ref for latest trending council)
        let fame = pd.baseFame || 0;
        const tc = trendingCouncilRef.current;
        if (tc) {
          const tb = evalCouncilObjective(tc, pd, true);
  ...
  function applyEffect(artist, pid, times = 1, stageIdx = -1) {
    ...
    if (!el.includes("year end") && !el.includes("roll 1 amenity dic")) {
