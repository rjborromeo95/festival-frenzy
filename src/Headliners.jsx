          ...
          for (const p of players) {
            ...
            (pd.stageArtists || []).forEach((sa, si) => sa.forEach((a, ai) => {
              ...
              for (let t = 0; t < times; t++) {
                ...
                if (eff.includes("round end: 1 vp / event")) {
                  // Count total events (global + personal) that affected this player
                ...
                if (eff.includes("round end: pick 1 artist to discard")) {
                  // Auto-pick highest VP card from hand to discard for VP
                  const hand = pd.hand || [];
                  if (hand.length > 0) {
                    const best = [...hand].sort((a, b) => b.vp - a.vp)[0];
                    vpBonus += best.vp;
                    addLog(p.festivalName, `${a.name}: Discarded ${best.name} from hand for +${best.vp} VP`);
                  }
                }
                if (eff.includes("end of this round: +2vp / negative event avoided") || eff.includes("+2vp / negative event avoided")) {
                  const evtRes = eventPhaseResults?.[p.id];
