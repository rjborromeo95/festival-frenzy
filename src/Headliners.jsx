...
const ALL_GENRES = ["Pop", "Rock", "Electronic", "Hip Hop", "Indie"];
...
const ALL_COUNCIL_OBJECTIVES = [
  ...
  { id: "groovin_circles", name: "Groovin' Circles", flavour: "The dance area will be cramped.", req: "Keep all tiles beside stage empty", benefit: "+2 tickets", tBenefit: "+1 Fame" },
  { id: "sniffer_dogs", name: "Sniffer Dogs", flavour: "Stamp down on illegal substances.", req: "2 security beside each other", benefit: "+1 VP/security adj to security", tBenefit: "+1 Fame/security adj to security" },
  { id: "special_sauce", name: "Special Sauce", flavour: "Keep herbs and spices secret.", req: "Security and catering next to each other", benefit: "+1 VP/security adj to catering", tBenefit: "+1 Fame/security adj to catering" },
  ...
  { id: "glamping", name: "Glamping", flavour: "Exclusive camping areas.", req: "Campsite and security next to each other", benefit: "+2 tickets/campsite adj to security", tBenefit: "+1 Fame/campsite adj to security" },
  ...
  { id: "number_one_fans", name: "Number One Fans", flavour: "More portaloos are better.", req: "Own more portaloos than stages", benefit: "+4 VP/3 portaloos", tBenefit: "+1 Fame/3 portaloos" },
  { id: "nimby", name: "NIMBY", flavour: "Keep the festival contained.", req: "Do not open stages", benefit: "+4 VP when declining stage", tBenefit: "+1 Fame when declining stage" },
  ...
  { id: "thieves_night", name: "Thieves in the Night", flavour: "Worried about thieves.", req: "Own more security than campsites", benefit: "+3 VP/3 security", tBenefit: "+1 Fame/3 security" },
  { id: "meat_the_law", name: "Meat the Law", flavour: "Look after the catering vans.", req: "Own more security than catering", benefit: "+3 VP/3 security", tBenefit: "+1 Fame/3 security" },
...
function evalCouncilObjective(obj, pd, isTrending) {
  ...
  switch (obj.id) {
    ...
    case "sniffer_dogs": count = ofType("security").filter(s => ofType("security").some(o => o !== s && areAdjacent(s.col, s.row, o.col, o.row))).length; break;
    case "special_sauce": count = ofType("security").filter(s => ofType("catering").some(c => areAdjacent(s.col, s.row, c.col, c.row))).length; break;
    ...
    case "glamping": count = ofType("campsite").filter(c => ofType("security").some(s => areAdjacent(c.col, c.row, s.col, s.row))).length; break;
    case "luxury_loos": count = ofType("portaloo").filter(p => ofType("security").some(s => areAdjacent(p.col, p.row, s.col, s.row))).length; break;
    case "number_one_fans": count = cnt("portaloo") > stages.length ? Math.floor(cnt("portaloo") / 3) : 0; break;
    case "nimby": count = 1; break;
    ...
    case "thieves_night": count = cnt("security") > cnt("campsite") ? Math.floor(cnt("security") / 3) : 0; break;
    case "meat_the_law": count = cnt("security") > cnt("catering") ? Math.floor(cnt("security") / 3) : 0; break;
  ...
  if (isTrending) { r.fame = count; return r; }

  switch (obj.id) {
    case "ticket_evaders": case "toxic_waste": case "put_a_lid": case "local_breweries": case "luxury_loos": r.tickets = count; break;
    ...
    case "sniffer_dogs": case "special_sauce": r.vp = count; break;
    case "in_n_out": r.tickets = count * 2; break;
    case "quiet_camping": r.vp = count; break;
    case "glamping": r.tickets = count * 2; break;
    case "number_one_fans": r.vp = count * 4; break;
    case "nimby": r.vp = 4; break;
    ...
    case "thieves_night": case "meat_the_law": r.vp = count * 3; break;
...
export default function Headliners() {
  ...
  const [undoSnapshot, setUndoSnapshot] = useState(null); // { playerData, dice, turnsLeft, turnAction, actionTaken, placingAmenity, movingFrom, type }
  ...
  const [undoSnapshot, setUndoSnapshot] = useState(null); // snapshot of state before last action
  ...
  function applyEffect(artist, pid, times = 1) {
    ...
    for (let t = 0; t < times; t++) {
      ...
      if (el.includes("+5 ticket sales")) {
        ...
        showFloatingBonus("+5 🎟️", "#fbbf24");
      }
      if (el.includes("+1 personal event")) {
        ...
        addLog("Effect", `${artist.name}: +1 Personal Event drawn`);
      }
      if (el.includes("+1 negative event")) {
        const negEvents = ALL_EVENTS.filter(e => e.color === "red");
        const drawn = negEvents[Math.floor(Math.random() * negEvents.length)];
        if (drawn) { setPlayerPersonalEvents(prev => ({ ...prev, [pid]: [...(prev[pid] || []), drawn] })); }
        addLog("Effect", `${artist.name}: +1 Negative Event drawn`);
      }
      if (el.includes("+1 global event")) {
        ...
        addLog("Effect", `${artist.name}: +1 Global Event added (${drawn?.color === "green" ? "🟢" : "🔴"})`);
      }
      ...
      if (el.includes("gain 2 vp whenever you play a security")) {
        setPlayerData(p => ({ ...p, [pid]: { ...p[pid], vpPerSecurity: (p[pid].vpPerSecurity || 0) + 2 } }));
        addLog("Effect", `${artist.name}: Gain 2 VP whenever you play a security this round!`);
        showFloatingBonus("👮‍♀️→⭐ Active!", "#c4b5fd");
      }
    }
  ...
  function evaluateObjective(obj, pd, isTrending) {
    if (!obj) return 0;
    const sa = pd.stageArtists || [];
    const genreTarget = { Popstars: "Pop", "Rock On": "Rock", "Disc Jockeys": "Electronic", "Fire Verses": "Hip Hop", Indiependent: "Indie" };

    // Check if objective is achieved on at least 1 stage
    let achieved = false;
    let stageCount = 0;

    if (obj.name === "Local Talent") {
      sa.forEach(s => { if (s.length === 3 && s[2].fame === 0) stageCount++; });
      achieved = stageCount >= 1;
    } else if (genreTarget[obj.name]) {
      const g = genreTarget[obj.name];
      sa.forEach(s => { if (s.length === 3 && s.every(a => getGenres(a.genre).includes(g))) stageCount++; });
      achieved = stageCount >= 1;
    } else if (obj.name === "Eclectic") {
      sa.forEach(s => { if (s.length === 3) { const gs = new Set(); s.forEach(a => getGenres(a.genre).forEach(g => gs.add(g))); if (gs.size >= 3) stageCount++; } });
      achieved = stageCount >= 1;
    } else if (obj.name === "Friends in Special Places") {
      sa.forEach(s => { if (s.length === 3) stageCount++; });
      achieved = stageCount >= 1;
    }

    // Trending: +1 Fame if achieved (returned as negative tickets to signal fame — handled by caller)
    if (isTrending) return achieved ? -999 : 0; // sentinel value, caller handles fame grant

    // Personal objective: tiered ticket rewards
    let bonus = 0;
    if (obj.name === "Local Talent") {
      if (stageCount >= 1) bonus += obj.rewards[1];
      if (stageCount >= 2) bonus += obj.rewards[2];
      if (stageCount >= 3) bonus += obj.rewards[3];
    } else if (genreTarget[obj.name]) {
      if (stageCount >= 1) bonus += obj.rewards[1];
      if (stageCount >= 2) bonus += obj.rewards[2];
      if (stageCount >= 3) bonus += obj.rewards[3];
  ...
  }, [phase, setupStep, setupIndex, currentPlayerIdx, showTurnStart, actionTaken, noTurnsLeft, pendingEffect, showHeadliner, showBookedArtist, showCouncilFame, showYearAnnouncement]);
  ...
  const handleGameHexClick = (col, row) => {
    ...
    if (turnAction === "pickAmenity" && placingAmenity) {
      ...
      setUndoSnapshot({ playerData: JSON.parse(JSON.stringify(playerData)), dice: [...dice], turnsLeft: { ...turnsLeft }, type: "placeAmenity" });
      ...
      if (!movingFrom) { const am = currentPD.amenities.find(a => a.col === col && a.row === row); if (am) setMovingFrom({ col, row, type: am.type }); }
        ...
        setUndoSnapshot({ playerData: JSON.parse(JSON.stringify(playerData)), dice: [...dice], turnsLeft: { ...turnsLeft }, type: "moveAmenity" });
  ...
  function confirmSecurityDelegation() {
    ...
    // Remove delegated security from player board (random positions)
    if (delegated > 0) {
      setPlayerData(p => {
    ...
    setEventPhaseResults(prev => ({ ...prev, [pid]: res }));
  ...
  const beginRoundEnd = () => {
    ...
    setPlayerData(prev => {
      ...
      for (const p of players) {
        ...
        if (personalObj) objTickets += evaluateObjective(personalObj, pd, false);
        // Trending: +1 Fame if achieved
        if (objTickets > 0) addLog(p.festivalName, `earned +${objTickets} tickets from artist objectives`);
        next[p.id] = { ...next[p.id], bonusTickets: (next[p.id].bonusTickets || 0) + objTickets };
      }
      return next;
    });
        ...
        setPlayerData(prev => {
          ...
          for (const p of players) {
            ...
            (pd.stageArtists || []).forEach((sa, si) => sa.forEach((a, ai) => {
              const eff = (a.effect || "").toLowerCase(); const times = 1;
              for (let t = 0; t < times; t++) {
                if (eff.includes("year end: 5 vp")) { vpBonus += 5; addLog(p.festivalName, `${a.name}: Year End → +5 VP${isHL ? " (headliner ×2)" : ""}`); }
                if (eff.includes("year end: 1 vp / fame")) { const fvp = pd.fame || 0; vpBonus += fvp; addLog(p.festivalName, `${a.name}: Year End → +${fvp} VP (1/Fame)`); }
                if (eff.includes("year end: 1 vp / event")) {
                  const evtRes = eventPhaseResults?.[p.id];
                  const totalEvts = evtRes ? (evtRes.positive.length + evtRes.negative.length) : 0;
                  vpBonus += totalEvts;
                  addLog(p.festivalName, `${a.name}: Year End → +${totalEvts} VP (1/Event)`);
                }
                if (eff.includes("year end: 2 vp / rock headliner")) { const rc = (pd.stageArtists || []).filter(s => s.length === 3 && getGenres(s[2].genre).includes("Rock")).length; vpBonus += 2 * rc; addLog(p.festivalName, `${a.name}: Year End → +${2*rc} VP (2/Rock HL)`); }
                if (eff.includes("year end: 2 vp / unique genres")) { const gs = new Set(); (pd.stageArtists || []).forEach(s => s.forEach(ar => getGenres(ar.genre).forEach(g => gs.add(g)))); vpBonus += 2 * gs.size; addLog(p.festivalName, `${a.name}: Year End → +${2*gs.size} VP (2/genre)`); }
                if (eff.includes("year end: pick 1 artist to discard")) {
                  const hand = next[p.id].hand || [];
                  if (hand.length > 0) {
                    const sorted = [...hand].sort((x, y) => y.vp - x.vp);
                    const bestCard = sorted[0];
                    vpBonus += bestCard.vp;
                    next[p.id] = { ...next[p.id], hand: hand.filter(h => h !== bestCard) };
                    addLog(p.festivalName, `${a.name}: Discarded ${bestCard.name} from hand → +${bestCard.vp} VP`);
                  }
                }
                if (eff.includes("end of this round") && eff.includes("negative event avoided")) {
                  const evtRes = eventPhaseResults?.[p.id];
  ...
  const anim = <style>{`@keyframes fadeSlideIn { from { opacity:0; transform:translateY(10px); } to { opacity:1; transform:translateY(0); } } @keyframes headlinerPulse { 0%,100% { transform:scale(1); } 50% { transform:scale(1.05); } } @keyframes affordPulse { 0%,100% { box-shadow: 0 0 4px rgba(251,191,36,0.3); } 50% { box-shadow: 0 0 16px rgba(251,191,36,0.7); } } .obj-hover-parent:hover .obj-hover-tip { display: block !important; max-height: 300px !important; padding: 10px !important; margin-top: 8px !important; opacity: 1 !important; } @keyframes floatUp { 0% { opacity:1; transform:translateY(0) scale(1); } 50% { opacity:1; transform:translateY(-30px) scale(1.2); } 100% { opacity:0; transform:translateY(-60px) scale(0.8); } } @keyframes bookReveal { 0% { opacity:0; transform:scale(0.5) rotate(-5deg); } 50% { transform:scale(1.1) rotate(2deg); } 100% { opacity:1; transform:scale(1) rotate(0deg); } }`}</style>;
  ...
  const exportGameData = () => {
    ...
    players.forEach(p => {
      ...
      for (let y = 1; y <= 4; y++) {
        const td = allTickets[p.id]?.[y];
        if (td && typeof td === "object") { row.push(td.raw, td.fame, td.mult, td.final); }
