...
export default function Headliners() {
  ...
  const showFloatingBonus = useCallback((text, color) => {
    ...
    setFloatingBonuses(p => [...p, { id, text, color: color || "#fbbf24" }]);
    setTimeout(() => setFloatingBonuses(p => p.filter(b => b.id !== id)), 2000);
  }, []);
  ...
  function applyEffect(artist, pid, times = 1) {
    ...
    for (let t = 0; t < times; t++) {
      ...
      if (el.includes("+1 security") && el.includes("place")) {
        // Queue a security placement for the player
        setPendingEffect({ type: "placeSpecific", amenityType: "security", artistName: artist.name });
        setPendingEffectPid(pid);
        addLog("Effect", `${artist.name}: +1 Security — place it on your board!`);
      } else if (el.includes("+1 security")) {
        // Auto-place security (non-interactive version for effects without "place this turn")
        setPendingEffect({ type: "placeSpecific", amenityType: "security", artistName: artist.name });
        setPendingEffectPid(pid);
        addLog("Effect", `${artist.name}: +1 Security — place it on your board!`);
      }
      if (el.includes("+1 amenity")) {
        // Queue amenity choice + placement
        setPendingEffect({ type: "placeAmenity", artistName: artist.name });
        setPendingEffectPid(pid);
        addLog("Effect", `${artist.name}: +1 Amenity — choose and place!`);
      }
      ...
      if (el.includes("+1 personal event")) {
        // Draw a random positive event as personal
      ...
      if (el.includes("sign 1 artist") || el.includes("sign one artist")) {
        // Queue sign from pool or deck
        setPendingEffect({ type: "signArtist", artistName: artist.name, canRefresh: el.includes("refresh") });
        setPendingEffectPid(pid);
        addLog("Effect", `${artist.name}: Sign 1 artist from pool or deck!`);
      }
      if (el.includes("draw two artists")) {
        // Draw 2, pick 1 to reserve
        const drawn = drawFromDeck(2);
        if (drawn.length > 0) {
          setPendingEffect({ type: "pickFromDrawn", drawn, artistName: artist.name });
          setPendingEffectPid(pid);
          addLog("Effect", `${artist.name}: Drew 2 artists — pick 1 to keep!`);
        }
      }
      ...
      if (el.includes("immediately book another")) {
        setPendingEffect({ type: "signArtist", artistName: artist.name, canRefresh: false });
        setPendingEffectPid(pid);
        addLog("Effect", `${artist.name}: Immediately book another artist!`);
      }
    }
    recalcTickets();
  }
  ...
  function bookArtistToStage(artist, stageIdx, pid) {
    ...
    if (artist.vp > 0) showFloatingBonus(`+${artist.vp} ⭐`, "#c4b5fd");
    if (artist.tickets > 0) showFloatingBonus(`+${artist.tickets} 🎟️`, "#fbbf24");
  ...
  const aiStep = useCallback(() => {
    ...
    if (pendingEffect && pendingEffectPid !== null) {
      ...
      if (pe.type === "signArtist") {
        if (artistPool.length > 0) {
          ...
          const np = [...artistPool]; np.splice(idx, 1); setArtistPool(np);
    ...
    if (phase === "game") {
      ...
      // Resolve die to amenity type
      let amenityType = dieVal;
      if (dieVal === "catering_or_portaloo") amenityType = Math.random() < 0.5 ? "catering" : "portaloo";
      else if (dieVal === "security_or_campsite") amenityType = Math.random() < 0.5 ? "security" : "campsite";
  ...
  const handleDiePick = (idx, dv) => {
    if (dv === "fame") {
      ...
      setPlayerData(p => ({ ...p, [currentPlayerId]: { ...p[currentPlayerId], baseFame: Math.min(FAME_MAX, (p[currentPlayerId].baseFame || 0) + 1) } }));
      addLog(currentPlayer.festivalName, `rolled 🔥 Fame! +1 Fame this round`);
    ...
    setPlayerData(prev => {
      ...
      for (const p of players) {
        ...
        if (trendingObjective) {
          ...
          if (tRes === -999) {
            next[p.id] = { ...next[p.id], baseFame: Math.min(FAME_MAX, (next[p.id].baseFame || 0) + 1) };
            addLog(p.festivalName, `📢 Achieved Trending "${trendingObjective.name}" → +1 🔥`);
          }
        }
  ...
  const startNextYear = () => {
    ...
    setPlayerData(prev => {
      ...
      for (const p of players) {
        ...
        next[p.id] = { ...pd, stageArtists: emptyStages, bonusTickets: 0, baseFame: 0 };
      }
      return next;
    });
  ...
  if (phase === "game") {
    ...
    return (<div style={CS}>{logBtn}{discardBtn}
      ...
      {/* Floating bonuses */}
      {floatingBonuses.map(fb => <div key={fb.id} style={{ position: "fixed", top: "40%", left: "50%", transform: "translateX(-50%)", zIndex: 999, pointerEvents: "none", animation: "floatUp 2s forwards" }}>
        ...
        if (pe.type === "placeSpecific") {
          return <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", zIndex: 960, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 20 }}>
            <div style={{ ...card, textAlign: "center", marginBottom: 12, padding: "12px 20px" }}>
              <p style={{ color: "#4ade80", margin: 0, fontSize: 14, fontWeight: 600 }}>✨ {pe.artistName}: Place your bonus {AMENITY_ICONS[pe.amenityType]} {AMENITY_LABELS[pe.amenityType]}!</p>
            </div>
        ...
        if (pe.type === "signArtist") {
          return <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", zIndex: 960, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
            <div style={{ ...card, textAlign: "center", maxWidth: 600, width: "100%" }}>
              <h3 style={{ color: "#4ade80", marginBottom: 12 }}>✨ {pe.artistName}: Sign 1 artist!</h3>
              <p style={{ color: "#8b5cf6", fontSize: 12, marginBottom: 12 }}>Pick an artist from the pool to add to your hand:</p>
              ...
              <div style={{ display: "flex", gap: 8, justifyContent: "center", flexWrap: "wrap" }}>
                {artistPool.map((a, i) => <ArtistCard key={i} artist={a} showCost small onClick={() => {
                  ...
                  setPendingEffect(null); setPendingEffectPid(null);
                  // Only refill the empty slot — no full refresh
                  setTimeout(() => refillPool(), 100);
                  setDeferPoolRefresh(false);
                }} />)}
              </div>
              <button onClick={() => {
                ...
                setPendingEffect(null); setPendingEffectPid(null);
                // If pool refresh was deferred and not used, refill empty slots
                if (deferPoolRefresh) { setTimeout(() => refillPool(), 100); setDeferPoolRefresh(false); }
        ...
        if (pe.type === "pickFromDrawn") {
          return <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", zIndex: 960, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
            <div style={{ ...card, textAlign: "center", maxWidth: 500 }}>
              <h3 style={{ color: "#4ade80", marginBottom: 12 }}>✨ {pe.artistName}: Pick 1 to keep!</h3>
              <div style={{ display: "flex", gap: 10, justifyContent: "center", flexWrap: "wrap" }}>
                {pe.drawn.map((a, i) => <ArtistCard key={i} artist={a} showCost onClick={() => {
                  setPlayerData(p => ({ ...p, [pid]: { ...p[pid], hand: [...p[pid].hand, a] } }));
      ...
      {showTurnStart && <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", zIndex: 900, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ ...card, textAlign: "center", maxWidth: 440, animation: "fadeSlideIn 0.3s" }}>
          ...
          {trendingObjective && <div style={{ marginTop: 12, padding: 10, borderRadius: 10, background: "rgba(251,191,36,0.08)", border: "1px solid rgba(251,191,36,0.25)" }}>
            ...
            <div style={{ fontSize: 10, color: "#fbbf24", marginTop: 4 }}>🔥 Achieve → +1 Fame</div>
          </div>}
        ...
        <div style={{ width: 220, padding: 16, borderRight: "1px solid #2a2a4a", overflowY: "auto", flexShrink: 0 }}>
          ...
          {trendingObjective && <div style={{ marginTop: 10, padding: 10, borderRadius: 10, background: "rgba(251,191,36,0.1)", border: "1px solid rgba(251,191,36,0.3)", position: "relative", cursor: "help" }} className="obj-hover-parent">
            ...
            <div className="obj-hover-tip" style={{ position: "relative", width: "100%", padding: 0, borderRadius: 10, maxHeight: 0, overflow: "hidden", transition: "all 0.2s", background: "#1a1a2e", border: "1px solid #fbbf24", boxShadow: "0 4px 20px rgba(0,0,0,0.5)", textAlign: "left" }}>
              ...
              <div style={{ fontSize: 10, color: "#fbbf24", fontWeight: 600, marginTop: 4 }}>🔥 Achieve this on any stage → +1 Fame</div>
            </div>
          </div>}
          {/* Personal Objective */}
