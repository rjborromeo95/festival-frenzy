...
const ALL_EVENTS = [
  ...
  { name: "Happy Campers", color: "green", desc: "More people in your campsites this year.", cond: null, result: "+1 ticket per campsite", apply: (pd) => ({ tickets: (pd.amenities?.campsite)||0 }) },
  ...
  { name: "Local Press Buzz", color: "green", desc: "Local press are raving about your festival.", cond: "has_stage", result: "+2 VP", apply: () => ({ vp: 2 }) },
  ...
  { name: "Bartenders with Flair", color: "green", desc: "Your catering vans juggle cocktail shakers.", cond: "catering_and_stage", result: "+1 VP/catering (up to stage count)", apply: (pd) => ({ vp: Math.min((pd.stages||[]).length, (pd.amenities?.catering)||0) }) },
  { name: "On their Best Behaviour", color: "green", desc: "Festival-goers were surprisingly nice.", cond: "security_and_stage", result: "+1 VP/pair of security and stage", apply: (pd) => ({ vp: Math.min((pd.stages||[]).length, (pd.amenities?.security)||0) }) },
  ...
  { name: "Sponsorship Deal", color: "green", desc: "A major brand wants to associate with your festival.", cond: "has_headliner", result: "+5 tickets", apply: () => ({ tickets: 5 }) },
  ...
  { name: "Powerful Gusts", color: "red", desc: "Campsites get battered by high winds.", cond: "has_campsite", result: "-3 VP", avoidable: true, apply: () => ({ vp: -3 }) },
  { name: "Flooding", color: "red", desc: "Campers wake up in a bog after heavy rain.", cond: "has_campsite", result: "-3 VP", avoidable: true, apply: () => ({ vp: -3 }) },
...
function eventConditionMet(evt, pd) {
  ...
  switch (evt.cond) {
    ...
    case "catering_and_stage": return cnt("catering") > 0 && (pd.stages||[]).length > 0;
    case "security_and_stage": return cnt("security") > 0 && (pd.stages||[]).length > 0;
...
export default function Headliners() {
  ...
  const startSetup = () => {
    const data = {}; players.forEach(p => { data[p.id] = { stages: [], amenities: { campsite: 0, security: 0, catering: 0, portaloo: 0 }, fame: 0, baseFame: 0, vpPerSecurity: 0, vp: 0, tickets: 0, rawTickets: 0, setupAmenity: null, hand: [], stageArtists: [], bonusTickets: 0, stageNames: [], stageColors: [] }; });
  ...
  const confirmSetupAmenity = () => {
    ...
    setPlayerData(p => ({
      ...
      [pid]: {
        ...p[pid],
        setupAmenity: setupSelectedAmenity,
        amenities: { ...p[pid].amenities, [setupSelectedAmenity]: (p[pid].amenities?.[setupSelectedAmenity] || 0) + 1 },
    ...
    addLog(currentSetupPlayer.festivalName, `chose ${AMENITY_LABELS[setupSelectedAmenity]} and stage`);
  ...
  const beginEventPhase = () => {
    ...
    players.forEach(p => {
      ...
      const secCount = (pd.amenities?.security) || 0;
  ...
  function confirmSecurityDelegation() {
    ...
    if (delegated > 0) {
      setPlayerData(p => {
        ...
        newAm.security = Math.max(0, (newAm.security || 0) - delegated);
        return { ...p, [pid]: { ...cur, amenities: newAm } };
      });
      ...
      try {
        ...
        if (fx.removeAmenity) {
          setPlayerData(p => {
            ...
            if ((ams[fx.removeAmenity] || 0) > 0) ams[fx.removeAmenity] = ams[fx.removeAmenity] - 1;
        ...
        if (fx.removeAmenityCount) {
          setPlayerData(p => {
            const ams = { ...(p[pid].amenities || {}) };
            const t = fx.removeAmenityCount.type;
            ams[t] = Math.max(0, (ams[t] || 0) - fx.removeAmenityCount.count);
            return { ...p, [pid]: { ...p[pid], amenities: ams } };
          });
        }
        if (fx.discardHand) {
