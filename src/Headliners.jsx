...
const ALL_COUNCIL_OBJECTIVES = [
  { id: "ticket_evaders", name: "Ticket Evaders", flavour: "The council are worried about people sneaking in.", req: "Security on the 2 outermost layers", benefit: "+1 ticket/security on edge, +2 VP if you move security to edge", tBenefit: "+1 Fame/security on edge" },
  { id: "toxic_waste", name: "Toxic Waste", flavour: "Locals are worried about strange smells.", req: "No portaloos on outside edge", benefit: "+1 ticket/portaloo not on edge, +2 VP if you move portaloo to edge", tBenefit: "+1 Fame/portaloo not on edge" },
  ...
  { id: "glamping", name: "Glamping", flavour: "Exclusive camping areas.", req: "Campsite, security and portaloo clustered together", benefit: "+2 tickets/campsite-security-portaloo cluster", tBenefit: "+1 Fame/campsite adj to security" },
  { id: "luxury_loos", name: "Luxury Loos", flavour: "The council like 2 ply toilet roll.", req: "Security in between 2 portaloos", benefit: "+2 tickets/portaloo-security-portaloo cluster", tBenefit: "+1 Fame/portaloo adj to security" },
  ...
  { id: "noise_complaints", name: "Noise Complaints", flavour: "Concern from neighbours.", req: "Move amenities nearer to centre", benefit: "+2 tickets and +2 VP per amenity moved inward", tBenefit: "+1 Fame/2 amenities not on edge" },
  ...
  { id: "thieves_night", name: "Thieves in the Night", flavour: "Worried about thieves.", req: "Campsites within 2 tiles of security", benefit: "+1 ticket/campsite within 2 tiles of security", tBenefit: "+1 Fame/3 security" },
  { id: "meat_the_law", name: "Meat the Law", flavour: "Look after the catering vans.", req: "Catering vans within 2 tiles of security", benefit: "+2 VP/protected catering van", tBenefit: "+1 Fame/3 security" },
  ...
  { id: "show_of_power", name: "Show of Power", flavour: "A lone guard commands respect.", req: "Security with no adjacent amenities", benefit: "+1 ticket/isolated security", tBenefit: "" },
...
function evalCouncilObjective(obj, pd, isTrending) {
  ...
  const bid = obj.id;
  if (bid === "ticket_evaders" || bid === "toxic_waste" || bid === "put_a_lid" || bid === "local_breweries" || bid === "number_one_fans" || bid === "thieves_night") r.tickets = count;
  else if (bid === "crowd_control") r.vp = Math.floor(count / 3) * 2;
  else if (bid === "food_courts") r.vp = count * 5;
  else if (bid === "groovin_circles") r.tickets = count * 2;
  else if (bid === "sniffer_dogs") r.vp = count;
  else if (bid === "special_sauce") r.tickets = count;
  else if (bid === "glamping") r.tickets = count * 2;
  else if (bid === "luxury_loos") r.tickets = count * 2;
  else if (bid === "in_n_out") r.tickets = count * 2;
  else if (bid === "quiet_camping") r.vp = count;
  else if (bid === "form_a_line") r.tickets = count >= 2 ? count - 1 : 0;
  else if (bid === "noise_complaints") r.tickets = count;
  else if (bid === "multiple_entrances") r.vp = count * 3;
  else if (bid === "reheating_fish") r.vp = count * 2;
  else if (bid === "eat_local") r.vp = count;
  else if (bid === "meat_the_law") r.vp = count * 2;
  else if (bid === "orderly_queues") r.tickets = count;
  else if (bid === "sit_downs") r.tickets = count * 3;
  else if (bid === "yours_or_mine") r.tickets = count * 5;
  else if (bid === "chef_beef") r.tickets = count;
  else if (bid === "show_of_power") r.tickets = count;
  else if (bid === "keep_the_peace") r.tickets = count;
  return r;
}
...
export default function Headliners() {
      ...
      if (!movingFrom) { const am = currentPD.amenities.find(a => a.col === col && a.row === row); if (am) setMovingFrom({ col, row, type: am.type }); }
        ...
        (playerCouncils[currentPlayerId] || []).forEach(co => {
          ...
          if (co.obj.id === "noise_complaints") {
            ...
            if (newDist < oldDist) {
              setPlayerData(p => ({ ...p, [currentPlayerId]: { ...p[currentPlayerId], vp: (p[currentPlayerId].vp || 0) + 2, bonusTickets: (p[currentPlayerId].bonusTickets || 0) + 2 } }));
              addLog(currentPlayer.festivalName, `📋 Noise Complaints: Moved amenity inward → +2 VP +2 tickets!`);
              showFloatingBonus("+2 ⭐ +2 🎟️", "#c4b5fd");
            }
          }
        });
  ...
  const proceedFromRoundEnd = () => {
    ...
    const f5 = players.filter(p => (playerData[p.id]?.fame || 0) >= 3);
    if (f5.length > 0) { setPreRoundIndex(0); setPreRoundStep("notify"); setDisplacedAmenities([]); setDisplacedPlaceIdx(0); setPhase("preRound"); }
    else startNextYear();
  };

  // Pre-round
  const preRoundPlayers = useMemo(() => players.filter(p => (playerData[p.id]?.fame || 0) >= 3), [players, playerData]);
  ...
  const handlePreRoundHexClick = (col, row) => {
    ...
    if (preRoundStep === "placeStage") {
      ...
      // Opening a new stage grants +1 Fame for the upcoming year
      setPlayerData(p => ({ ...p, [pid]: { ...p[pid], baseFame: Math.min(FAME_MAX, (p[pid].baseFame || 0) + 1) } }));
