          ...
          {!isMobile && <>
            ...
            {sidebarTab === "my" && <div style={{ marginTop: 6 }}>
              {playerObjectives[currentPlayerId] && (() => { const obj = playerObjectives[currentPlayerId]; const { count } = countGenreLineups(obj, currentPD); return <div style={{ padding: 8, borderRadius: 8, background: "rgba(124,58,237,0.1)", border: "1px solid rgba(124,58,237,0.3)", marginBottom: 6 }}>
                <div style={{ fontSize: 9, fontWeight: 700, color: "#c4b5fd", textTransform: "uppercase" }}>🎯 Objective</div>
                <div style={{ fontSize: 10, fontWeight: 700, color: "#e9d5ff" }}>{obj.name}</div>
                {count > 0 && <div style={{ fontSize: 9, color: "#4ade80" }}>✅ {count} qualifying</div>}
              </div>; })()}
              {(playerCouncils[currentPlayerId] || []).map((co, ci) => { const res = evalCouncilObjective(co.obj, currentPD, false); return <div key={ci} style={{ padding: 6, borderRadius: 6, marginBottom: 4, background: res.count > 0 ? "rgba(34,197,94,0.15)" : "rgba(124,58,237,0.06)", border: `1px solid ${res.count > 0 ? "#22c55e50" : "#4c1d9540"}` }}>
                ...
                <div style={{ fontSize: 9, color: res.count > 0 ? "#4ade80" : "#64748b" }}>{res.tickets > 0 ? `+${res.tickets} 🎟️` : "Not yet"}</div>
              </div>; })}
            ...
            {sidebarTab === "trending" && <div style={{ marginTop: 6 }}>
              {trendingCouncil && <div style={{ padding: 8, borderRadius: 8, background: "rgba(34,197,94,0.08)", border: "1px solid rgba(34,197,94,0.3)", marginBottom: 6 }}>
                ...
                <div style={{ fontSize: 9, color: "#94a3b8" }}>{trendingCouncil.req}</div>
              </div>}
              {microtrends.map((mt, i) => { const claimed = mt.claimedBy !== null; return <div key={i} style={{ padding: 4, borderRadius: 6, marginBottom: 3, background: claimed ? "rgba(107,114,128,0.1)" : `${GENRE_COLORS[mt.genre]}15`, opacity: claimed ? 0.5 : 1 }}>
                <div style={{ fontSize: 10, fontWeight: 700, color: claimed ? "#6b7280" : GENRE_COLORS[mt.genre] }}>{claimed ? "✓" : "🔥"} {mt.genre}</div>
              </div>; })}
            </div>}
            {sidebarTab === "goals" && <div style={{ marginTop: 6 }}>
              {activeGoals.map((ag, gi) => { const g = ag.goal; const prog = goalProgress[currentPlayerId]?.[g.trackKey] || 0; return <div key={gi} style={{ padding: 6, borderRadius: 6, marginBottom: 4, background: "rgba(251,191,36,0.06)", border: "1px solid rgba(251,191,36,0.2)" }}>
                <div style={{ fontSize: 10, fontWeight: 700, color: "#fbbf24" }}>🏆 {g.name}</div>
                <div style={{ fontSize: 9, color: prog >= g.target ? "#4ade80" : "#c4b5fd" }}>{prog >= g.target ? "✅" : "🎯"} {prog}/{g.target}</div>
              </div>; })}
