  ...
  function trackGoalProgress(pid, trackKey) {
    setGoalProgress(prev => {
      ...
      activeGoals.forEach((ag, gi) => {
        ...
        if (newVal >= ag.goal.req2Target && !goalClaimsRef.current[`${ag.goal.id}_req2`]) {
        ...
        if (newVal >= ag.goal.req3Target && !goalClaimsRef.current[`${ag.goal.id}_req3`]) {
      ...
      (pd.stageArtists || []).forEach(sa => sa.forEach(a => {
        try {
          ...
          if (eff.includes("vp if you have the highest fame")) {
            const isHighest = players.every(op => op.id === p.id || calcFame(snap[op.id] || {}, playerCouncils[op.id] || []) <= fame);
