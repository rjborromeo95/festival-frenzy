    ...
    try {
      ...
      setPlayerData(prev => {
        ...
        for (const p of players) {
          ...
          (playerCouncils[p.id] || []).forEach(co => { if (co.active) { vpBonus += calcCouncilBenefit(co.obj, pd, false).vp; } });
          (pd.stageArtists || []).forEach(sa => sa.forEach(a => {
            try {
              ...
              if (eff.includes("vp if you have the highest fame")) { if (players.every(op => op.id === p.id || calcFame(next[op.id] || {}, playerCouncils[op.id] || []) <= fame)) { vpBonus += 1; if (players.every(op => op.id === p.id || (next[op.id]?.tickets || 0) <= rawT)) vpBonus += 3; } }
              ...
              if (eff.includes("vp / council objective")) vpBonus += (playerCouncils[p.id] || []).filter(co => evalCouncilObjective(co.obj, pd, false).count > 0).length;
            ...
            } catch(err) { console.error("YE err:", a?.name, err); }
          }));
          if (!nat[p.id]) nat[p.id] = {};
          nat[p.id][year] = { raw: rawT, fame, fameVP, ticketVP };
          addLog(p.festivalName, `🎟️ ${rawT}→${ticketVP}VP | 🔥${fame}→${fameVP}VP | Artists+${artistVP}VP`);
