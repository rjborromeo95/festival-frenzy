...
export default function Headliners() {
  ...
  const startNextYear = () => {
    ...
    players.forEach(p => {
      ...
      setPlayerCouncils(prev => {
        const next = { ...prev };
        for (const p of players) {
          next[p.id] = (next[p.id] || []).map(co => ({ ...co, fameGranted: false }));
        }
        return next;
      });
    });
          ...
          {(playerCouncils[currentPlayerId] || []).length > 0 && <div style={{ marginTop: 8 }}>
            ...
            {(playerCouncils[currentPlayerId] || []).map((co, ci) => (
              <div key={ci} style={{ padding: 6, borderRadius: 8, marginBottom: 4, background: co.active ? "rgba(34,197,94,0.15)" : "rgba(239,68,68,0.1)", border: `1px solid ${co.active ? "#22c55e50" : "#ef444450"}`, position: "relative", cursor: "help" }} className="obj-hover-parent">
                ...
                <div style={{ fontSize: 9, color: "#94a3b8" }}>{co.obj.benefit}</div>
                <div className="obj-hover-tip" style={{ position: "relative", width: "100%", padding: 0, borderRadius: 10, maxHeight: 0, overflow: "hidden", transition: "all 0.2s", background: "#1a1a2e", border: `1px solid ${co.active ? "#22c55e" : "#ef4444"}`, boxShadow: "0 4px 20px rgba(0,0,0,0.5)", textAlign: "left" }}>
                  ...
                  <div style={{ fontSize: 10, color: "#4ade80" }}>Passive: {co.obj.benefit}</div>
                  <div style={{ fontSize: 9, color: "#94a3b8", marginTop: 4 }}>Status: {co.active ? "Active ✅" : "Inactive ❌"}</div>
                </div>
              </div>
            ))}
          </div>}
