...
export default function Headliners() {
  ...
  function confirmSecurityDelegation() {
    ...
    if (!pid || !eventPhaseResults) return;
      ...
      setTimeout(() => {
        ...
        setPlayerData(prev => {
          ...
          for (const p of players) {
            ...
            const fameVP = FAME_VP[Math.min(5, pd.fame || 0)] || 0;
            ...
            (pd.stageArtists || []).forEach((sa, si) => sa.forEach((a, ai) => {
              ...
              for (let t = 0; t < times; t++) {
                if (eff.includes("year end: 5 vp")) { vpBonus += 5; addLog(p.festivalName, `${a.name}: Year End → +5 VP`); }
            ...
            nat[p.id][year] = { raw: rawT, fame: pd.fame, fameVP, ticketVP };
            addLog(p.festivalName, `🎟️ ${rawT} tickets → ${ticketVP} VP | 🔥 Fame ${pd.fame} → ${fameVP} VP`);
            next[p.id] = { ...pd, tickets: rawT, rawTickets: rawT, vp: (pd.vp || 0) + vpBonus };
          }
          return next;
        });
        setAllTickets(nat);
