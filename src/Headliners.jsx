  ...
  if (phase === "objectiveChoice" && pendingObjectiveChoice) {
    ...
    if (isAI && pendingObjectiveChoice.options.length > 0) {
      setTimeout(() => {
        const pick = pendingObjectiveChoice.options[Math.floor(Math.random() * pendingObjectiveChoice.options.length)];
        chooseObjective(pick);
        // Check who still needs objectives (use current pid since chooseObjective just added to them)
        const currentPid = pendingObjectiveChoice.playerId;
        const nextP = players.find(p => p.id !== currentPid && (playerObjectives[p.id] || []).length === 0);
        if (nextP) {
          offerObjectiveChoice(nextP.id);
        } else {
          setPendingObjectiveChoice(null);
          ...
          <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap" }}>
            {pendingObjectiveChoice.options.map((obj, i) => (
              <div key={i} onClick={() => {
                ...
                else { setPendingObjectiveChoice(null); setTimeout(() => setPhase("game"), 200); }
